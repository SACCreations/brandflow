      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {['All', 'Pending', 'Approved', 'Rejected'].map((f) => (
            <button
              key={f}
              className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:border-brand-400 hover:text-brand-600 dark:border-gray-700 dark:text-gray-400 transition-colors first:bg-brand-50 first:border-brand-400 first:text-brand-700"
            >
              {f}
            </button>
          ))}
