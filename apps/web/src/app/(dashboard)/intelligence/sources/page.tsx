...
export default function SourcesPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {isError ? (
        <div className="flex flex-col items-center justify-center p-12 bg-red-50 dark:bg-red-500/5 rounded-3xl border border-red-100 dark:border-red-900/20">
          <AlertTriangle className="h-10 w-10 text-red-500 mb-4" />
          <h3 className="text-lg font-bold text-red-900 dark:text-red-400">Failed to load sources</h3>
          <p className="text-sm text-red-600/70 mb-6">There was an error connecting to the intelligence engine.</p>
          <button onClick={() => refetch()} className="px-6 py-2 bg-red-100 text-red-700 rounded-xl font-bold text-xs hover:bg-red-200 transition-all">Try Again</button>
        </div>
      ) : (
