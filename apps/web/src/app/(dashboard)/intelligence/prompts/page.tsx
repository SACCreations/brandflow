  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-700">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Prompt Engine</h1>
          <p className="mt-2 text-gray-500 dark:text-gray-400">Manage, version, and route the intelligence layer of your brand brain.</p>
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className={`rounded-xl p-2 ${
                    prompt.layer === 'platform' ? 'bg-gray-100 text-gray-600 dark:bg-gray-800' :
                    prompt.layer === 'brand' ? 'bg-brand-50 text-brand-600 dark:bg-brand-500/10' :
                    'bg-purple-50 text-purple-600 dark:bg-purple-500/10'
                  }`}>
                    <Layers className="h-5 w-5" />
                  </div>
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-white">{prompt.name}</h3>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-sm font-bold text-gray-900 dark:text-white">{prompt.score}%</div>
                    <div className="text-[10px] text-gray-400 font-bold uppercase">Perf Score</div>
                  </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="rounded-2xl border border-brand-100 bg-brand-50/20 p-6 dark:border-brand-500/20 dark:bg-brand-500/5">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-center"><ChevronRight className="h-4 w-4 rotate-90 text-gray-300" /></div>
...
function RuleItem({ label, active }: any) {
  return (
    <div className="flex items-center justify-between">
      ...
      <div className={`h-2 w-8 rounded-full ${active ? 'bg-emerald-500' : 'bg-gray-200'}`}></div>
    </div>
  );
}
