  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-700">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Publishing Calendar</h1>
          <p className="mt-2 text-muted-foreground">See every queued, published, and failed post on a real calendar instead of hoping time is a flat list.</p>
        </div>
        <Link href="/publish" className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-foreground hover:bg-brand-700">
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-background border-border bg-background">
        <div className="flex items-center justify-between border-b border-border px-6 py-4 border-border">
          <h2 className="font-semibold text-foreground">{monthName} {year}</h2>
          <div className="flex gap-2">
            <button onClick={() => setMonthOffset((value) => value - 1)} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:bg-surface-1 bg-background border-border dark:hover:bg-surface-1">
        ...
        <div className="grid grid-cols-7 divide-x divide-y divide-gray-100 dark:divide-gray-800">
          {days.map((day) => (
            <div key={day} className="bg-surface-1 bg-background px-3 py-3 text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground bg-surface-2/50">
              {day}
            </div>
          ))}
          ...
          {cells.map((day, index) => {
            ...
            return (
              <div
                key={index}
                className={`min-h-[150px] p-3 align-top ${isToday ? 'bg-brand-50/40 dark:bg-brand-900/10' : 'hover:bg-surface-1 dark:bg-gray-950/60 dark:hover:bg-surface-1/20'}`}
              >
                {day !== null ? (
                  <>
                    <span className={`inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${isToday ? 'bg-primary text-foreground' : 'text-muted-foreground text-foreground'}`}>
                      {day}
                    </span>

                    <div className="mt-3 space-y-2">
                      {daySchedules.slice(0, 3).map((schedule) => (
                        <div key={schedule.id} className="rounded-xl border border-border/60 bg-background/80 p-2 text-xs border-border dark:bg-gray-950/40">
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-semibold text-foreground">{schedule.socialAccount.name}</span>
                          ...
                          <div className="mt-1 line-clamp-2 text-[11px] text-muted-foreground text-foreground">
                      ...
                      {daySchedules.length > 3 ? (
                        <Link href="/publish" className="block text-[11px] font-semibold text-primary hover:underline">
