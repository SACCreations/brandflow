...
export default function VariantComparisonPage() {
  ...
  if (!groupId) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-4 text-center">
        <h3 className="text-lg font-bold text-foreground">No variant group specified</h3>
        <p className="text-sm text-muted-foreground">Navigate here from batch generation results to compare variants.</p>
  ...
  if (isError || variants.length === 0) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-4 text-center">
        <h3 className="text-lg font-bold text-foreground">No variants found</h3>
        <p className="text-sm text-muted-foreground">This generation group has no content items to compare.</p>
        <Link href="/create/content">
  ...
  return (
    ...
    <div className="mx-auto max-w-7xl px-2 py-4 space-y-8 animate-in fade-in duration-500">
      {/* Header */}
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-border/60 pb-6 border-border">
        <div className="space-y-2">
          <Link href="/create/content" className="inline-flex items-center gap-2 text-xs font-semibold text-muted-foreground hover:text-primary transition-colors">
            <ArrowLeft className="h-4 w-4" /> Back to Generator
          </Link>
          <div className="flex items-center gap-3">
            <div className="rounded-xl bg-primary/10 p-2.5 dark:bg-primary/100/10">
              <BarChart3 className="h-7 w-7 text-primary dark:text-brand-400" />
            </div>
            <div>
        ...
        <Button
          ...
          className="gap-2 bg-primary hover:bg-brand-700 text-foreground font-bold px-6 py-2.5 rounded-xl shadow-lg"
        >
          ...
          return (
            <Card
              ...
              className={`relative flex flex-col overflow-hidden transition-all cursor-pointer ${
                isWinner
                  ? 'ring-2 ring-primary/20 border-primary shadow-lg shadow-brand-500/10'
                  : 'border-border hover:border-gray-300 border-border dark:hover:border-gray-700'
              }`}
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-border/60 p-4 border-border">
                <div className="flex items-center gap-3">
                  <div className={`flex h-8 w-8 items-center justify-center rounded-full ${isWinner ? 'bg-primary text-foreground' : 'border-2 border-border text-muted-foreground'}`}>
                    {isWinner ? <CheckCircle2 className="h-4 w-4" /> : <span className="text-xs font-bold">{variants.indexOf(variant) + 1}</span>}
                  </div>
                  <div>
                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{variant.platform}</span>
                    <p className="text-[10px] text-muted-foreground">{new Date(variant.createdAt).toLocaleString()}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 p-5">
                <p className="text-sm text-foreground leading-relaxed text-foreground line-clamp-[12]">
                  {variant.body}
                </p>
              </div>

              {/* Footer Actions */}
              <div className="flex items-center justify-between border-t border-border/60 p-3 border-border">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="rounded-lg min-h-[44px] min-w-[44px] flex items-center justify-center text-muted-foreground hover:bg-surface-2 hover:text-gray-600 dark:hover:bg-surface-1"
                  >
                    <RotateCcw className="h-4 w-4" />
                  </button>
                  <Link href={`/create/content/${variant.id}`} onClick={(e) => e.stopPropagation()}>
                    <button aria-label="Open in editor" className="rounded-lg min-h-[44px] min-w-[44px] flex items-center justify-center text-muted-foreground hover:bg-surface-2 hover:text-gray-600 dark:hover:bg-surface-1">
      ...
      <Card className="p-5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4">Quick Metrics Comparison</h3>
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {variants.map((variant, idx) => (
            <div key={variant.id} className="space-y-2 text-center">
              <span className="text-[10px] font-bold text-muted-foreground uppercase">Variant {idx + 1}</span>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  <div className="text-lg font-black text-foreground">{variant.body.split(' ').length}</div>
                  <div className="text-[9px] text-muted-foreground uppercase">Words</div>
                </div>
                <div>
                  <div className="text-lg font-black text-foreground">{variant.body.length}</div>
                  <div className="text-[9px] text-muted-foreground uppercase">Chars</div>
                </div>
                <div>
