      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          {settings.map((setting) => (
            <div key={setting.id} className="space-y-4 p-6 rounded-3xl bg-white dark:bg-gray-900/40 border border-transparent shadow-2xl shadow-gray-200/40 dark:shadow-none hover:shadow-lg transition-all group ring-1 ring-gray-200/50 dark:ring-white/10 backdrop-blur-xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-xl bg-brand-50 dark:bg-brand-900/40 text-brand-600">
                    <Type className="w-4 h-4" />
                  </div>
                  <div>
                    <h3 className="text-xs font-black uppercase tracking-widest text-gray-900 dark:text-white leading-none">{setting.label}</h3>
                    <p className="text-[9px] font-medium text-gray-400 mt-1">Google Font or System Font</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="space-y-1.5 relative">
                  <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 ml-1">Font Family</span>
                  <div className="relative">
                    ...
                    <Input 
                      ...
                      className="h-12 pl-10 rounded-2xl bg-gray-50 dark:bg-gray-950/50 dark:bg-gray-800/30 border-gray-200 dark:border-gray-800/60 dark:border-gray-700/50 shadow-inner font-bold text-sm"
                    />
                ...
                <div className="space-y-1.5">
                  <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 ml-1">Weight</span>
                  <Select value={setting.weight || '400'} onValueChange={(val) => onChange(settings.map(s => s.id === setting.id ? { ...s, weight: val } : s))}>
                    <SelectTrigger className="h-12 rounded-2xl bg-gray-50 dark:bg-gray-950/50 dark:bg-gray-800/30 border-gray-200 dark:border-gray-800/60 dark:border-gray-700/50 shadow-inner">
              ...
              <div className="pt-2">
                <div className="flex items-center gap-2 mb-2 ml-1">
                   <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">CSS Fallback Stack</span>
                </div>
                <code className="block w-full p-3 rounded-xl bg-gray-900 text-gray-300 text-[10px] font-mono whitespace-nowrap overflow-x-auto shadow-inner">
                   font-family: {getFallbackStack(setting.fontFamily || 'sans-serif')};
                </code>
              </div>

              <div className="p-6 rounded-2xl bg-gray-50 dark:bg-gray-950/50 dark:bg-gray-800/20 border border-gray-100 dark:border-gray-800/50 mt-4 overflow-hidden relative min-h-[120px] flex items-center justify-center shadow-inner">
                <p 
                ...
                <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2 py-1 rounded-full bg-white dark:bg-gray-900/80 dark:bg-gray-900/80 backdrop-blur-md shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        <div className="space-y-6">
          <div className="p-8 rounded-3xl bg-brand-600 text-white shadow-2xl shadow-brand-500/20 dark:shadow-none space-y-4 ring-1 ring-brand-400/30 overflow-hidden relative">
            ...
            <div className="relative z-10 pt-4">
              <Button className="w-full bg-white dark:bg-gray-900 text-brand-600 hover:bg-brand-50 rounded-2xl text-[10px] font-black uppercase tracking-widest h-12 shadow-lg" disabled={!hasDetectedPairing}>
                {hasDetectedPairing ? 'Keep Detected Typography' : 'Await Source Signals'}
              </Button>
            </div>
          </div>

          <div className="p-8 rounded-3xl border border-transparent dark:border-transparent bg-white dark:bg-gray-900/40 shadow-2xl shadow-gray-200/40 dark:shadow-none space-y-6 ring-1 ring-gray-200/50 dark:ring-white/10 backdrop-blur-xl">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400">Scale Governance</h3>
            <div className="space-y-4">
              {scales.map((item, i) => (
                <div key={item.id} className="group p-4 rounded-2xl hover:bg-gray-50 dark:bg-gray-950/50 dark:hover:bg-gray-800/30 transition-all border border-transparent hover:border-gray-200 dark:border-gray-800/60 dark:hover:border-gray-700/50">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex flex-col flex-1 mr-4">
                      <Input 
                        value={item.label} 
                        onChange={(e) => onScaleChange(scales.map(s => s.id === item.id ? { ...s, label: e.target.value } : s))}
                        className="h-8 text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-tight bg-transparent border-none p-0 focus-visible:ring-0 placeholder:text-gray-300"
                        placeholder="Scale Name"
                      />
                    </div>
                    <Badge variant="outline" className="text-[8px] font-black h-5 px-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white dark:bg-gray-900">EDITABLE</Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative">
                      <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[8px] font-black text-gray-400 uppercase">Size</div>
                      <Input 
                        value={item.size} 
                        onChange={(e) => onScaleChange(scales.map(s => s.id === item.id ? { ...s, size: e.target.value } : s))}
                        className="h-10 text-[11px] font-bold bg-gray-50 dark:bg-gray-950/50 dark:bg-gray-800/30 border-gray-200 dark:border-gray-800/60 dark:border-gray-700/50 rounded-xl pl-10 text-gray-900 dark:text-white shadow-inner"
                        placeholder="e.g. 16px"
                      />
                    </div>
                    <div className="relative">
                      <div className="absolute left-3 top-1/2 -translate-y-1/2 text-[8px] font-black text-gray-400 uppercase">LS</div>
                      <Input 
                        value={item.spacing} 
             ...
             <div className="pt-4">
                <Button 
                  type="button" 
                  variant="outline" 
                  className="w-full rounded-2xl text-[10px] font-black uppercase tracking-widest h-12 border-dashed border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:bg-gray-950 dark:hover:bg-gray-800 hover:border-gray-400 dark:hover:border-gray-500"
