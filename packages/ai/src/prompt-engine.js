...
class PromptEngine {
     ...
     */
    buildSystemPrompt(resolved, context) {
    ...
    injectBrandContext(template, brand) {
        return template
            ...
            .replace(/\{\{banned_phrases\}\}/g, brand.governance?.bannedPhrases?.join(', ') ?? 'None')
            .replace(/\{\{required_phrases\}\}/g, brand.governance?.requiredPhrases?.join(', ') ?? 'None')
            .replace(/\{\{cta_preferences\}\}/g, brand.governance?.ctaPreferences?.join(', ') ?? 'None');
    }
}
