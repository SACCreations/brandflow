...
class QualityControl {
     ...
     */
    async check(content, brand, knowledgeFacts = [], options) {
        const violations = [];
        const citations = [];
    ...
    async evalCompliance(content, brand, apiKey) {
...
Respond in JSON format:
{
  ...
  "violations": [
    ...
    async evalFactuality(content, brand, facts, apiKey) {
        if (facts.length === 0)
...
Respond in JSON format:
{
  ...
  "citations": [
    ...
    async evalSafety(content, apiKey) {
...
Respond in JSON: {"score": 0.0-1.0, "violations": [{"type": "unsafe_content", "severity": "critical", "detail": "string"}]}`;
    ...
    calculateGrade(score, violations) {
